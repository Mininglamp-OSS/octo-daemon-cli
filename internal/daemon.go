...
func (d *Daemon) deregister() {
	...
	// row to clear. best-effort — shutdown proceeds regardless. Uses its OWN
	// context so a slow/stuck daemon-deregister can't burn the shutdown window
	// and starve the runtime deregister below.
	dctx, dcancel := context.WithTimeout(context.Background(), 5*time.Second)
	if err := d.client.DaemonDeregister(dctx, d.daemonID); err != nil {
