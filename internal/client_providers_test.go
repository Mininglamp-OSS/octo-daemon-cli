...
func TestListProviders_ParsesActiveList(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/daemon/runtime-providers" {
		...
		// fleet wkhttp c.Response(data):无 outer envelope
		_, _ = w.Write([]byte(`{"providers":[{"name":"claude","display_name":"Claude","binary_name":"claude","upgrade_timeout_sec":600},{"name":"openclaw","display_name":"OpenClaw","binary_name":"openclaw","upgrade_timeout_sec":720}]}`))
	}))
	defer srv.Close()
